...
public class UserService {
  ...
  public UserDto saveUser(UserDto userDto) {
    ...
    UserDto referenceDataUser = userReferenceDataService.putUser(userDto);
    if (referenceDataUser != null) {
      userDto.setId(referenceDataUser.getId());
    }

    User dbUser = userRepository.findOneByReferenceDataUserId(userDto.getId());

    boolean isNewUser = dbUser == null;
    if (isNewUser) {
      dbUser = new User();
    }
    dbUser.updateFrom(userDto);
  ...
  public void sendResetPasswordEmail(User user, String email, boolean isNewUser) {
    UserDto referenceDataUser = userReferenceDataService.findUser(user.getUsername());
