...
## Service Design
The function of the Auth Service is to 1) generate access tokens for other Services to use, and 2) check access tokens for other Services and return the basic access level of the token provided.

There are two types of access tokens that can be generated by the Auth Service:
* User-based - this access token is associated with a User (a concept in the OpenLMIS system), whose rights can be checked in the Role-Based Access Control mechanism in OpenLMIS.
* Service-based - this access token is not associated with a User, and is strictly for Service-to-Service communication. Only Services should use such tokens because they essentially give "root" level access in the system; all endpoints are accessible for them.
