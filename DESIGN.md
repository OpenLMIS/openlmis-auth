...
The API Key is generated by the Reference Data Service. The Auth service takes part of this process but related auth's endpoint should not be used directly. Additionally the endpoint accepts only service level token which should be from the Reference Data Service.
